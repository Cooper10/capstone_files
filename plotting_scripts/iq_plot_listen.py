...
Potential issue with soundfile module to install, please do the following
pip install soundfile

open a new python script and use:
help("modules") and confirm that soundfile is listed
If not, see https://pypi.org/project/SoundFile/ for more directions
...
def iq_to_wav(records, save_directory, sequences_to_play=32, stereo=True):
    """
    ...
    print("\n \n Record Length: " + str(len(records)) + "\n \n")
    print("Total Length of Records: " + str(len(records)))
    counter = 0
...
def plot_iq(records, save_directory, sequences_to_plot=32):
    """
    ...
    for record_num, record in enumerate(records):
        ...
        if sequences_to_plot is None:
            # Plot all of them!
            #print("Sequences to plot: " + str(record['seqnum']))
        ...
        data = data.reshape((int(record['seqnum']), int(record['chnnum']),
                             int(record['smpnum']), 2))
        print("Data sequences: " + str(np.size(data, 0)))
        if sequences_to_plot > np.size(data, 0) and sequences_to_plot is None:
            # mismatch between the 'seqnum' variable and the data array length... happens sometimes
            sequences_to_plot = np.size(data, 0)
            print("Sequences: " + str(sequences_to_plot))
        ...
        for sequence in range(sequences_to_plot):
            ...
            if sequence == 0:
                # compute range gate distance and add it into the title
            ...
            #ax.plot(sample_numbers[0:len(phase_diff)], phase_diff, 'k')
            ax.plot(sample_numbers, iq_imag, 'r')
            ax.plot(sample_numbers, iq_real, 'b')
        ...
        if record_num % int((num_records / percent_update)) == 0:
            ...
            print("Finished record {}/{}".format(record_num + 1, num_records))
        rate = round(1e6/record['smsep']/1000,2)
        plt.xlabel('Sample Number with rate: ' + str(rate) + ' kHz')   # adding a more descriptive x label (CR)
        fig = plt.gcf()
